    ...
    <section class="checkout checkout-step" :class="[`checkout-${stepId}`, { active: isActive }]"
        ...
        <div class="row">
                                ...
                                <td data-label="Product" style="vertical-align: middle">
                                    <div
                                        ...
                                        <div class="product-info pl-2">
                                            ...
                                            <p class="mb-0 product-title single-Product-view">
                                                {{ product.product_name }}
                                            </p>
                                            <p class="mb-0"><b>Product Code: </b>{{ product.product_code }}</br></p>
                                            <p class="text-danger font-italic">{{ product.note }}</p>
                                        </div>
                                    </div>
                                </td>

                                <td data-label="Quantity" class="text-right font-weight-bold"
                                    style="vertical-align: middle;">{{
                                        product.qty }}</td>

                                <td data-label="Quantity" class="text-right font-weight-bold"
                                    style="vertical-align: middle;">{{
            priceFormatter(product.price)}} / {{product.custom_item_info?.uom  }}</td>

                                <td data-label="Subtotal" class="text-right font-weight-bold"
                                    style="vertical-align: middle;">{{
            priceFormatter(product.custom_item_info?.total_price) }}</td>
                            </tr>
                            ...
                            <tr v-for="(product, index) in products" :key="index"
                                ...
                                <td data-label="Product" style="vertical-align: middle">
                                    <div
                                        class="product-thumb d-flex gap-1 justify-content-end justify-content-md-start">
                                        <a :href="product.url" class="product-thumbnail">
                                            <div v-if="product.source" :id="`product-badge-${index}`"
                                                class="product-badge badge-sm mt-2 ml-2"
                                                :class="getBadgeType(product.source_type)" data-toggle="tooltip"
            ...
            product.qty }}</td>
                                <td data-label="Quantity" class="text-right font-weight-bold"
                                    style="vertical-align: middle;">{{
            priceFormatter(product.price) + "/" + product.uom }}</td>
                                <td data-label="Subtotal" class="text-right font-weight-bold"
                                    style="vertical-align: middle;">{{
            priceFormatter(product.subtotal) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div class="shopping-cart-footer">
                    ...
                    <div class="column text-lg">Subtotal: <span class="text-medium">{{ sub_total }}</span></div>
                </div>
                <div class="row padding-top-1x mt-3 shipping-lg">
                    <div class="col-sm-6">
                        <h5>Shipping to:</h5>
                        <ul class="list-unstyled">
                            <li><span class="text-muted">Name :</span> {{ getName }}</li>
                    ...
                    <section class="widget widget-order-summary p-2">
                        ...
                        <table class="table">
                            <tr>
                             ...
                             <tr>
                                <td>Shipping:</td>
                                <td class="text-medium">$ {{ $parent.shippingMethod?.amount ?? 0 }}</td>
                            </tr>
                            <tr>
                                <td>Estimated tax:</td>
...
export default {
    ...
    computed: {
        ...
        getCustomerAddress() {
            let addressOne = this.$parent.getCustomerAddress.addressOne != null ? `${this.$parent.getCustomerAddress.addressOne}, ` : '';
            let addressTwo = this.$parent.getCustomerAddress.addressTwo != null ? `${this.$parent.getCustomerAddress.addressTwo},` : '';
            let addressThree = this.$parent.getCustomerAddress.addressThree != null ? `${this.$parent.getCustomerAddress.addressThree},` : '';
            let country = this.$parent.getCustomerAddress.country != null ? `${this.$parent.getCustomerAddress.country?.name}` : '';
            let state = this.$parent.getCustomerAddress.state != null ? `${this.$parent.getCustomerAddress.state?.name},` : '';
            let zipCode = this.$parent.getCustomerAddress.zipCode != null ? `${this.$parent.getCustomerAddress.zipCode},` : '';
    ...
    methods: {
        async fetchCartItems() {
            this.cart = await fetch('/get/carts').then(res => res.json()).then(res => res.data);
