...
let aRange = {low:24,heigh:450}
let bRange = {low:42,heigh:900}
...
export const checkInputValue = (event,voltage) => {
  ...
  if (formats.integer.test(depth)) {
    return validateInterValue(depth,voltage) ?? getInteger(depth);
  } else if (formats.decimal.test(depth)) {
    return validateInterValue(depth,voltage) ?? getDecimal(depth);
  } else if (formats.fractionWithSpace.test(depth)) {
    let data = getFractionWithSpace(depth);
    return validateFractionWithSpace(depth, data,voltage) ?? data;
  } else if (formats.fractionWithHyphen.test(depth)) {
    let data = getFractionWithHyphen(depth);
    return validateFractionWithHyphen(depth, data,voltage) ?? data;
  } else {
...
function validateInterValue(data,voltage) {
  return checkIntegerNumber(data,voltage);
}

function validateFractionWithSpace(depth, data,voltage) {
  const [val, fract] = depth.split(" ");
  if (Object.values(fractional).indexOf(data.fract) == -1) {
    return { 'error': "Invailate Number" }
  }else {
    return checkIntegerNumber(val,voltage);
  }
}

function validateFractionWithHyphen(depth, data,voltage) {
  const [val, fract] = depth.split("-");
  if (Object.values(fractional).indexOf(data.fract) == -1) {
    return { 'error': "Invailate Number" }
  } else {
...
function checkIntegerNumber(val,voltage){
  let range = voltage=='115'?aRange:bRange;
  if (val < range.low) {
    return { 'error': `Value must be getter than or equal to ${range.low}` }
  } else if (val > range.heigh) {
    return { 'error': `Value must no be getter than ${range.heigh}` }
  }
}
