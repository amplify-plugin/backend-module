	...
	<section class="container-fluid d-print-none">
		...
		<h2>
	        ...
	        @if ($crud->hasAccess('list') && !in_array($crud->entity_name_plural, listOfCrudToHideBackToButton()))
	          <small class=""><a href="{{ $menu_url }}" class="font-sm"><i class="la la-angle-double-left"></i> {{ trans('backpack::crud.back_to_all') }} <span>{{ $crud->entity_name_plural }}</span></a></small>
	        @endif
	    </h2>
    </section>
@endsection
	    ...
	    <div class="card no-padding no-border">
			<table class="table table-striped mb-0">
		        ...
		        @foreach ($crud->columns() as $column)
                    <tr>
		                <td>
		                    <strong>{!! ucwords($column['label']) !!}:</strong>
		                </td>
                        <td>
