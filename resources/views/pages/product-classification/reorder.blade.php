...
@section('content')
    ...
    function tree_element($entry, $key, $all_entries, $crud)
    {
        ...
        if (!isset($entry->tree_element_shown)) {
            ...
            echo '<li '.checkType($entry, $all_entries).' id="list_' . $entry->getKey() . '">';
            echo '<div>
                    <span class="disclose">
                        <span></span>
				   </span>
    ...
    function checkType($entry, $all_entries)
    {
        if($all_entries->where('parent_id', $entry->id)->count()) {
            return 'class="mjs-nestedSortable-branch mjs-nestedSortable-expanded"';
        }
    ...
    <script type="text/javascript">
        jQuery(document).ready(function ($) {
            ...
            $('#toArray').click(function (e) {
                ...
                let arraied = sliceIntoChunks($('ol.sortable').nestedSortable('toArray', {startDepthCount: 0}), 100);

                // log it
                // console.log(arraied);

                // send it with POST
                let requests = arraied.map(function(data) {
                    return $.ajax({
            ...
            perPage.on('change', function () {
                window.location.href = window.location.pathname + '?page=1&items=' + this.value;
            });
