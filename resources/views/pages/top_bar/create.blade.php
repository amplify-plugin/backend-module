...
@php
    $breadcrumbs = [
        ...
        'Top Bar' => backpack_url('top-bar'),
        Route::is('top-bar.edit') ? 'Edit' : (Route::is('top-bar.show') ? 'Preview' : 'Create') => false,
    ];
...
@section('header')
    <section class="container-fluid">
        <h2>
            ...
            <small>
                {!! $crud->getSubheading() ?? trans('backpack::crud.'.(Route::is('top-bar.edit') ? 'edit' : (Route::is('top-bar.show') ? 'preview' : 'add'))).' '.$crud->entity_name !!}
                .</small>
...
@section('content')
    <top-bar-create
        url="{{ url($crud->getCurrentOperation() === 'update'
                        ? $crud->route.'/'.$entry->getKey().'/edit' : $crud->route.'/create') }}"
        ...
        local_top_bar="{{json_encode($top_bar ?? new \stdClass())}}"
        active_topbar_widgets="{{$active_topbar_widgets ?? json_encode([])}}"
    ></top-bar-create>
@endsection
